...
// Download a video from a URL to a temp file on disk
async function downloadVideoFromUrl(videoUrl) {
  ...
  try {
    ...
    throw new Error(`Invalid video_url: ${e.message}`);
  }
  const tmpName = `dl_${crypto.randomBytes(8).toString("hex")}.mp4`;
  const tmpPath = path.join(uploadDir, tmpName);
  console.log(`[download] ${parsedUrl.hostname}${parsedUrl.pathname.slice(0, 60)} …`);
  const fetchRes = await fetch(videoUrl, { redirect: "follow", headers: { "User-Agent": "StoryloadBot/1.0" } });
  if (!fetchRes.ok) throw new Error(`Download failed: HTTP ${fetchRes.status} from ${parsedUrl.hostname}`);
  const writer = fs.createWriteStream(tmpPath);
  for await (const chunk of fetchRes.body) writer.write(chunk);
  await new Promise((resolve, reject) => { writer.end(); writer.on("finish", resolve); writer.on("error", reject); });
  const { size } = fs.statSync(tmpPath);
  if (size === 0) { fs.unlinkSync(tmpPath); throw new Error("Downloaded file is empty — check the URL"); }
  console.log(`[download] Done: ${(size / 1024 / 1024).toFixed(1)} MB → ${tmpName}`);
  return { filePath: tmpPath, fileSize: size, originalName: path.basename(parsedUrl.pathname) || "video.mp4" };
}

// Middleware that accepts both multipart/form-data (file upload) and application/json (video_url)
function flexUpload(req, res, next) {
  const ct = req.headers["content-type"] || "";
  if (ct.startsWith("multipart/form-data")) return upload.single("video")(req, res, next);
...
async function publishInBackground(pubId, { platform, ch, filePath, fileSize, originalName, videoUrl, title, description, privacy }) {
  ...
  try {
    ...
    if (platform === "tiktok") {
      const result = await doPublishTikTok(ch, { title: title || originalName, filePath, fileSize });
    ...
    else if (filePath) try { fs.unlinkSync(filePath); } catch {}
    await query("UPDATE sl_publications SET status='failed',error_message=$1,updated_at=NOW() WHERE id=$2", [e.message, pubId]).catch(() => {});
    console.error(`[bg] pub=${pubId} FAILED:`, e.message);
  }
}
...
<header class="header">
  <div class="header-inner">
    ...
    <div class="header-right">
      ${user ? `
        <a href="/download" class="btn-outline" style="padding:6px 14px;font-size:13px;display:inline-flex;align-items:center;gap:6px">
          <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v13m0 0l-4-4m4 4l4-4M3 20h18"/></svg>
          Download
        </a>
...
app.post("/v1/profiles/:id/publish", requireApiKey, flexUpload, async (req, res) => {
  ...
  if (!hasFile && !hasUrl) {
    return res.status(400).json({ error: "bad_request", message: "Provide a video file (multipart field 'video') or a JSON body with 'video_url'" });
  }
  ...
  try {
    ...
    if (!chs[0]) { if (req.file) fs.unlinkSync(req.file.path); return res.status(400).json({ error: "channel_not_connected", message: `${platform} channel is not connected to this profile` }); }
    const ch = chs[0];
    ...
    res.status(202).json({
      publication: formatPublication(pub),
      message: `Publication created. Video is being processed. Poll GET /v1/publications/${pub.id} for status.`,
    });

    publishInBackground(pub.id, {
      platform, ch,
...
// POST /v1/publications — create publication and publish async.
...
app.post("/v1/publications", requireApiKey, flexUpload, async (req, res) => {
  ...
  // Validate early — before touching DB
  if (!platform || !ALLOWED_PLATFORMS.has(platform)) {
    if (req.file) fs.unlinkSync(req.file.path);
    return res.status(400).json({ error: "bad_request", message: "platform is required: tiktok or youtube" });
  }
  if (wantPublish && !hasFile && !hasUrl) {
    return res.status(400).json({ error: "bad_request", message: "Provide a video file (multipart field 'video') or a JSON body with 'video_url'. Use status=draft to save without video." });
  }
  ...
  try {
    ...
    // Validate channel exists before creating the record
    let ch;
    if (wantPublish) {
      if (profile_id) {
      ...
      if (!ch) {
        if (req.file) fs.unlinkSync(req.file.path);
        return res.status(400).json({ error: "channel_not_connected", message: `${platform} channel is not connected. Connect it first on the dashboard.` });
      }
    }
    ...
    // 202 immediately — client must poll GET /v1/publications/:id
    res.status(202).json({
      publication: formatPublication(pub),
      message: `Publication created. Video is being processed. Poll GET /v1/publications/${pub.id} for status.`,
    });
...
function safeFilename(str) {
  return (str || "video").replace(/[^\w\s\-().]/g, "").trim().slice(0, 120) || "video";
}

// GET /download — downloader page
app.get("/download", requireAuth, async (req, res) => {
  const user = await getUser(req.session.userId);
  res.send(layout("Download YouTube", `
  <div class="page-wrap">
    <div class="page-content" style="max-width:600px">
      <h1 class="page-title">Download YouTube Video</h1>
      <p class="page-sub" style="margin-bottom:28px">Paste a YouTube link and download the video as MP4 to your computer.</p>

      <div class="dl-box">
        <div style="display:flex;gap:10px;align-items:stretch">
          <input id="yt-url" type="url" placeholder="https://www.youtube.com/watch?v=..." class="modal-input" style="flex:1;margin:0;font-size:14px" oninput="resetUI()" onkeydown="if(event.key==='Enter')fetchInfo()">
          <button class="btn-primary" onclick="fetchInfo()" id="info-btn" style="white-space:nowrap;min-width:100px">
            <span id="info-btn-text">Get Video</span>
          </button>
        </div>

        <div id="dl-info" style="display:none;margin-top:24px;padding-top:24px;border-top:1px solid var(--border)">
          <div style="display:flex;gap:16px;align-items:flex-start;margin-bottom:20px">
            <img id="dl-thumb" src="" alt="" style="width:120px;height:68px;object-fit:cover;border-radius:8px;flex-shrink:0;background:var(--border)">
            <div style="min-width:0">
              <div id="dl-title" style="font-size:15px;font-weight:600;line-height:1.4;color:#fff;word-break:break-word;margin-bottom:6px"></div>
              <div id="dl-meta" style="font-size:12px;color:var(--muted)"></div>
            </div>
          </div>

          <label style="display:block;font-size:12px;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;font-weight:600;margin-bottom:8px">Format & Quality</label>
          <div id="dl-formats" style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px"></div>

          <button class="btn-primary" id="dl-btn" onclick="startDownload()" style="width:100%;justify-content:center;gap:8px;font-size:15px;padding:12px">
            <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v13m0 0l-4-4m4 4l4-4M3 20h18"/></svg>
            <span id="dl-btn-text">Download MP4</span>
          </button>
          <div id="dl-note" style="font-size:11px;color:var(--muted);text-align:center;margin-top:10px">The file will be saved to your Downloads folder</div>
        </div>

        <div id="dl-error" style="display:none;margin-top:20px;padding:12px 16px;background:rgba(255,80,80,.1);border:1px solid rgba(255,80,80,.3);border-radius:10px;font-size:13px;color:#ff8080"></div>
      </div>
    </div>
  </div>

  <style>
    .dl-box { background: rgba(255,255,255,.04); border: 1px solid var(--border); border-radius: 16px; padding: 28px; }
    .fmt-btn { padding: 7px 14px; border-radius: 8px; border: 1px solid var(--border); background: rgba(255,255,255,.06); color: var(--sub); font-size: 13px; cursor: pointer; transition: all .15s; font-family: inherit; }
    .fmt-btn.selected { border-color: var(--green); color: var(--green); background: rgba(52,213,154,.08); }
    .fmt-btn:hover { border-color: rgba(255,255,255,.3); color: #fff; }
  </style>
  ...
  async function fetchInfo() {
    ...
    try {
      const r = await fetch('/api/yt-info?url=' + encodeURIComponent(url));
      const data = await r.json();
      if (!r.ok) { showError(data.error || 'Failed to fetch video info'); return; }
      document.getElementById('dl-title').textContent = data.title;
      document.getElementById('dl-thumb').src = data.thumbnail;
      document.getElementById('dl-meta').textContent = data.author + (data.duration ? ' · ' + data.duration : '');
      const fmts = document.getElementById('dl-formats');
      fmts.innerHTML = '';
      selectedItag = null;
      data.formats.forEach((f, i) => {
        const b = document.createElement('button');
        b.className = 'fmt-btn' + (i === 0 ? ' selected' : '');
        b.textContent = f.label;
        b.dataset.itag = f.itag;
        if (i === 0) selectedItag = f.itag;
        b.onclick = () => {
      ...
      document.getElementById('dl-info').style.display = 'block';
    } catch(e) { showError('Network error. Please try again.'); }
    finally { btn.disabled = false; btnText.textContent = 'Get Video'; }
  }
  ...
  function startDownload() {
    ...
    if (selectedItag) params.set('itag', selectedItag);
    ...
    setTimeout(() => { btnText.textContent = 'Download MP4'; btn.disabled = false; }, 2000);
  }
  </script>
  `, user));
});

// GET /api/yt-info — return video metadata + available formats
app.get("/api/yt-info", requireAuth, async (req, res) => {
  const { url } = req.query;
  ...
  try {
    ...
    // Build a list of combined (video+audio) formats, sorted by quality
    ...
    const seen = new Set();
    const formats = combined.reduce((acc, f) => {
      const label = f.qualityLabel || (f.height ? `${f.height}p` : "?");
      if (!seen.has(label)) {
        seen.add(label);
        acc.push({ itag: f.itag, label: `${label} · MP4` });
      }
      return acc;
    }, []);

    // Fallback: if no combined formats, offer best video+audio (muxed by ytdl itself)
    if (formats.length === 0) {
      formats.push({ itag: null, label: "Best quality · MP4" });
    }
    ...
    const thumb = details.thumbnails && details.thumbnails.length
...
// GET /api/yt-download — stream video to client
app.get("/api/yt-download", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const opts = { filter: "videoandaudio" };
    if (itag) opts.quality = parseInt(itag, 10);
    ...
    stream.on("error", (err) => {
      console.error("[yt-download stream]", err.message);
...
<div class="docs-layout">
  ...
  <span class="hl-key">"offset"</span>: <span class="hl-num">0</span>
    ...
    <div class="docs-section" id="pub-create">
      ...
      <p class="docs-p">Create a publication and start publishing asynchronously. The server always responds <strong>202 Accepted</strong> immediately — the upload happens in the background. Poll <code>GET /v1/publications/:id</code> until <code>status</code> is <code>published</code> or <code>failed</code>.</p>
      <p class="docs-p">Use <code>status=draft</code> to save metadata without triggering a publish.</p>
      <div class="endpoint">
        <div class="endpoint-head">
          <span class="method post">POST</span>
          <span class="endpoint-path">/v1/publications</span>
          <span class="endpoint-desc">multipart or JSON with video_url</span>
        </div>
        <div class="endpoint-body">
          <div class="docs-alert">
            <svg width="16" height="16" fill="none" stroke="#34D59A" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            Two ways to provide a video: upload a file (<code>multipart/form-data</code>) or pass a public URL (<code>application/json</code> with <code>video_url</code>). The server downloads the URL automatically — no large file transfer needed.
          </div>
          <div class="docs-h3">Option A — Upload file (multipart/form-data)</div>
          <table class="param-table">
          ...
          <div class="docs-h3">Option B — Video URL (application/json)</div>
          <table class="param-table">
            <thead><tr><th>Field</th><th>Type</th><th>Description</th></tr></thead>
            <tbody>
              <tr><td>platform<span class="req">*</span></td><td>string</td><td><code>youtube</code> or <code>tiktok</code></td></tr>
              <tr><td>video_url<span class="req">*</span></td><td>string</td><td>Public HTTP/HTTPS URL to the video. Server downloads it. Required unless <code>status=draft</code></td></tr>
              <tr><td>title<span class="opt">opt</span></td><td>string</td><td>Title (max 100 chars for YouTube, 150 for TikTok)</td></tr>
              <tr><td>description<span class="opt">opt</span></td><td>string</td><td>Description (YouTube only, max 5000 chars)</td></tr>
              <tr><td>privacy<span class="opt">opt</span></td><td>string</td><td>YouTube: <code>public</code> · <code>private</code> · <code>unlisted</code>. Default: <code>private</code></td></tr>
              <tr><td>profile_id<span class="opt">opt</span></td><td>string</td><td>Profile UUID. Omit to use the default channel</td></tr>
          ...
          <div class="docs-h3">Example — publish by URL</div>
          <div class="code-block">
            <div class="code-block-head"><span class="code-lang">curl</span><button class="copy-btn" onclick="copyCode(this)">Copy</button></div>
            <pre>curl -X POST ${escHtml(baseUrl)}/v1/publications \\
  -H <span class="hl-str">"Authorization: Bearer sl_live_..."</span> \\
  ...
  -d <span class="hl-str">'{
    ...
    "privacy": "public"
          ...
          <div class="docs-h3">Response — 202 Accepted (always immediate)</div>
          <div class="resp-block"><pre>{
  <span class="hl-key">"publication"</span>: {
    ...
    <span class="hl-key">"video_id"</span>: <span class="hl-str">null</span>,
    <span class="hl-key">"video_url"</span>: <span class="hl-str">null</span>,
    <span class="hl-comment">/* ... rest of fields */</span>
  },
  <span class="hl-key">"message"</span>: <span class="hl-str">"Publication created. Poll GET /v1/publications/9b1deb... for status."</span>
}</pre></div>
          <div class="docs-h3">Poll until published</div>
          <div class="code-block">
            <div class="code-block-head"><span class="code-lang">curl — polling</span><button class="copy-btn" onclick="copyCode(this)">Copy</button></div>
            <pre><span class="hl-comment"># Poll every 5s until status is "published" or "failed"</span>
curl ${escHtml(baseUrl)}/v1/publications/<span class="hl-str">PUBLICATION_ID</span> \\
  -H <span class="hl-str">"Authorization: Bearer sl_live_..."</span></pre>
          </div>
          <div class="docs-h3">Published response (after processing)</div>
          <div class="resp-block"><pre>{
  <span class="hl-key">"publication"</span>: {
    <span class="hl-key">"id"</span>: <span class="hl-str">"9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d"</span>,
    <span class="hl-key">"platform"</span>: <span class="hl-str">"youtube"</span>,
